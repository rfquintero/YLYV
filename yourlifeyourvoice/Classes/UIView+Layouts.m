...
-(void)centerInBounds:(CGRect)bounds offsetX:(CGFloat)offsetX offsetY:(CGFloat)offsetY {
    [self centerInBounds:bounds offsetX:offsetX offsetY:offsetY thatFits:CGSizeUnbounded];
}

-(void)centerInBounds:(CGRect)bounds offsetX:(CGFloat)offsetX offsetY:(CGFloat)offsetY thatFits:(CGSize)size {
    CGSize sz = [self sizeThatFits:size];
    self.frame = CGRectMake(offsetX + roundf((bounds.size.width-sz.width)/2), offsetY + roundf((bounds.size.height - sz.height)/2), sz.width, sz.height);
}
